...
public class DataInitializer implements CommandLineRunner {
    ...
    private static final String CAMPUS_CENTRAL = "Campus Central";
    private static final String DEGREE = "Licenciatura";
    ...
    private void seedUsers() {
        ...
        userRepository.save(User.builder()
                ...
                .role(ERole.ADMIN)
                .build());
        if (userRepository.existsByUsername(CAPTURISTA_USERNAME)) return;
        userRepository.save(User.builder()
                .username(CAPTURISTA_USERNAME)
    ...
    private void seedSenders() {
        ...
        senderRepository.save(Sender.builder().name("Licenciatura en Informatica").campus(CAMPUS_CENTRAL).build());
        senderRepository.save(Sender.builder().name("Coordinacion de Posgrado").campus(CAMPUS_CENTRAL).build());
        senderRepository.save(Sender.builder().name("Instituto de Investigacion").campus("Campus Sur").build());
        senderRepository.save(Sender.builder().name("Departamento de Extension Universitaria").campus(CAMPUS_CENTRAL).build());
        LOG.info("Emisores creados");
    }

    private void seedReceivers() {
        if (receiverRepository.count() > 0) return;
        receiverRepository.save(Receiver.builder().name("Juan").lastName("Perez").twoLastName("Garcia").phone("5551234567").email("juan.perez@example.com").academicGrade(DEGREE).build());
        ...
        receiverRepository.save(Receiver.builder().name("Ana").lastName("Gonzalez").twoLastName("Sanchez").phone("5554443322").email("ana.gonzalez@example.com").academicGrade(DEGREE).build());
        ...
        receiverRepository.save(Receiver.builder().name("Pedro").lastName("Morales").twoLastName("Vega").phone("5558889900").email("pedro.morales@example.com").academicGrade(DEGREE).build());
